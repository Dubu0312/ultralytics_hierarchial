...
"""
test_yolo.py
Evaluate a saved hierarchical YOLO checkpoint on a test set and (optionally)
...
class YOLOTestDataset(Dataset):
    def __init__(self, df: pd.DataFrame, imgsz: int = 224):
        ...
        for ri, r_idx in enumerate(rows_chunks):
            for ci, c_idx in enumerate(cols_chunks):
                ...
                _plot_cm(sub_norm, sub_names_c,
                         ...
                         figscale_per_label=figscale, dpi=dpi, annotate=annotate)
        # Also save "full" overview without annotations
        _plot_cm(cm, names,
...
# -------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate hierarchical YOLO classification model on test set")
    p.add_argument("--ckpt", type=str, default="../best_model.pt", help="Path to best_model.pt")
    p.add_argument("--base_model", type=str, default="yolo11x-cls.pt", help="Base YOLO model (same as training)")
    p.add_argument("--root_test", type=str, default="/home/dubu/manh/dongvan-yolo/dataset-dongvan-train/test", help="Test data folder")
    p.add_argument("--mapping_csv", type=str, default="/home/dubu/manh/dongvan-yolo/pollen_dong_van.csv", help="Mapping CSV file")
    p.add_argument("--id_name_csv", type=str, default="", help="Optional ID↔name mapping CSV")
    ...
    p.add_argument("--out_csv", type=str, default="test_predictions_yolo.csv", help="Output predictions CSV")
    p.add_argument("--confmat_dir", type=str, default="cm_out_test_yolo", help="Directory for confusion matrices")

    # CM sizing/tiling knobs
    p.add_argument("--cm-figscale", type=float, default=0.80, help="Inches per label for CM figure size")
    ...
    p.add_argument("--cm-chunk-size", type=int, default=None, help="Tile CM into blocks of this many labels")
...
def main():
    ...
    base_model = ckpt.get("base_model", args.base_model)
    ...
    print(f"Classes - Order: {ncls['ordor']}, Family: {ncls['familia']}, Genus: {ncls['genus']}, Species: {ncls['species']}")
    ...
    # Dataset & loader
    test_ds = YOLOTestDataset(df_test, imgsz=args.imgsz)
    test_dl = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False,
                         num_workers=args.num_workers, pin_memory=True)

    # Model
    print(f"Loading model with base: {base_model}")
    model = YOLOHierarchicalClassifier(base_model, ncls["ordor"], ncls["familia"], ncls["genus"], ncls["species"])
    ...
    # Print metrics
    print("\n" + "=" * 100)
    print(f"[TEST RESULTS] masked={args.masked}")
    print(f"  ordor:   acc={metrics['ordor']['acc']:.3f}, f1_macro={metrics['ordor']['f1_macro']:.3f}, f1_weighted={metrics['ordor']['f1_weighted']:.3f}")
    print(f"  familia: acc={metrics['familia']['acc']:.3f}, f1_macro={metrics['familia']['f1_macro']:.3f}, f1_weighted={metrics['familia']['f1_weighted']:.3f}")
    print(f"  genus:   acc={metrics['genus']['acc']:.3f}, f1_macro={metrics['genus']['f1_macro']:.3f}, f1_weighted={metrics['genus']['f1_weighted']:.3f}")
    print(f"  species: acc={metrics['species']['acc']:.3f}, f1_macro={metrics['species']['f1_macro']:.3f}, f1_weighted={metrics['species']['f1_weighted']:.3f}")
    print(f"  hierarchical accuracy: {metrics['hier']:.3f}")
    print("=" * 100)

    # Save predictions table
    pd.DataFrame(rows).to_csv(args.out_csv, index=False, encoding="utf-8")
    print(f"\nSaved detailed predictions to: {args.out_csv}")
    ...
    pd.DataFrame(metrics_rows).to_csv("metrics_summary_yolo.csv", index=False, encoding="utf-8")
    print("Saved metrics_summary_yolo.csv")

    # Confusion matrices (optional)
    if args.confmat_dir:
        print(f"\nGenerating confusion matrices in: {args.confmat_dir}")
