...
"""
Hierarchical YOLO Classification Training Script (Masked CE Variant).

Same as train_hierarchical.py but uses masked cross-entropy at training:
for each level below order, child logits are masked to only those whose
ground-truth parent matches the GT parent label, before computing CE.
This enforces hierarchy consistency during training (Cách A — hard constraint).
...
Usage:
  python train_hierarchical_masked.py --data /path/to/dataset --mapping_csv /path/to/mapping.csv --epochs 100
"""
...
class YOLOHierarchicalClassifier(nn.Module):
    ...
    def __init__(self, base_model_path: str, n_ordor: int, n_familia: int, n_genus: int, n_species: int):
        ...
        self.backbone = yolo_model.model.model[:-1]
...
# Loss & Metrics
# =========================
def multitask_ce_masked(logits, targets, lambdas, familia_parent, genus_parent, species_parent):
    """Masked CE: mask child logits to only those whose parent matches GT parent, then CE.

    Args:
        logits: tuple of (lo, lf, lg, ls) — raw logits for order/family/genus/species
        targets: dict with keys 'ordor', 'familia', 'genus', 'species'
        lambdas: dict of per-level weights
        familia_parent: [n_fam] tensor, familia_parent[i] = parent ordor of family i
        genus_parent: [n_gen] tensor
        species_parent: [n_spe] tensor

    Returns:
        total: scalar loss
        parts: tuple of per-level loss values (floats)
    """
    lo, lf, lg, ls = logits

    # Order: top of hierarchy, no masking
    Lo = F.cross_entropy(lo, targets["ordor"])

    # Family: only compete among families whose parent == GT order
    # mask_f[b, i] = True iff familia_parent[i] == targets["ordor"][b]
    mask_f = (familia_parent.unsqueeze(0) == targets["ordor"].unsqueeze(1))
    lf_masked = lf.masked_fill(~mask_f, -1e9)
    Lf = F.cross_entropy(lf_masked, targets["familia"])

    # Genus: only compete among genera whose parent == GT family
    mask_g = (genus_parent.unsqueeze(0) == targets["familia"].unsqueeze(1))
    lg_masked = lg.masked_fill(~mask_g, -1e9)
    Lg = F.cross_entropy(lg_masked, targets["genus"])

    # Species: only compete among species whose parent == GT genus
    mask_s = (species_parent.unsqueeze(0) == targets["genus"].unsqueeze(1))
    ls_masked = ls.masked_fill(~mask_s, -1e9)
...
@torch.no_grad()
def masked_decode(lo, lf, lg, ls, familia_parent, genus_parent, species_parent):
    """Top-down masked decoding: mask child logits by predicted parent before argmax."""
    o = lo.argmax(1)
    # Family: mask to families whose parent == predicted order
    mask_f = (familia_parent.unsqueeze(0) == o.unsqueeze(1))
    f = lf.masked_fill(~mask_f, -1e9).argmax(1)
    # Genus: mask to genera whose parent == predicted family
    mask_g = (genus_parent.unsqueeze(0) == f.unsqueeze(1))
    g = lg.masked_fill(~mask_g, -1e9).argmax(1)
    # Species: mask to species whose parent == predicted genus
    mask_s = (species_parent.unsqueeze(0) == g.unsqueeze(1))
    s = ls.masked_fill(~mask_s, -1e9).argmax(1)
    return o, f, g, s


@torch.no_grad()
def evaluate(model, loader, device, familia_parent, genus_parent, species_parent):
    """Evaluate model on validation set using masked decoding (matches how the model was trained).

    Args:
        model: hierarchical classifier
        loader: validation DataLoader
        device: torch device
        familia_parent, genus_parent, species_parent: parent lookup tensors (on device)
    """
    model.eval()
    o_pred, o_true = [], []
    f_pred, f_true = [], []
    ...
    for x, y in loader:
        ...
        o_hat, f_hat, g_hat, s_hat = masked_decode(
            lo, lf, lg, ls, familia_parent, genus_parent, species_parent
        )
        o_pred += o_hat.cpu().tolist()
        o_true += y["ordor"].cpu().tolist()
        f_pred += f_hat.cpu().tolist()
        f_true += y["familia"].cpu().tolist()
        g_pred += g_hat.cpu().tolist()
        g_true += y["genus"].cpu().tolist()
        s_pred += s_hat.cpu().tolist()
...
# =========================
def main():
    parser = argparse.ArgumentParser(description="Train hierarchical YOLO classification model (MASKED CE)")
    ...
    parser.add_argument("--output", type=str, default="outputs_yolo_hierarchical_masked", help="Output directory")
    ...
    print("=" * 100)
    print("[MASKED TRAINING] Hierarchy-constrained CE loss enabled")
    print("  - training: family/genus/species logits masked by GT parent before cross-entropy")
    print("  - validation: top-down masked decoding (mask by PREDICTED parent) — matches training")
    print("=" * 100)
    ...
    # Move parent arrays to device once (used in every batch)
    familia_parent_dev = familia_parent.to(device)
    genus_parent_dev = genus_parent.to(device)
    species_parent_dev = species_parent.to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x, y in train_dl:
            ...
            logits = model(x)
            loss, parts = multitask_ce_masked(
                logits, y, LAMBDA,
                familia_parent_dev, genus_parent_dev, species_parent_dev,
            )
    ...
    print("=" * 100)
    print(f"Training (masked CE) completed at epoch {epoch}! Best hierarchical accuracy: {best_hier:.3f}")
